...
class onMemberJoin(commands.Cog):
    def __init__(self, bot):
        ...
        self.db = self.client['discord-bot-vii-rpg']['discord-bot-vii-rpg']
    ...
    @commands.command(
        ...
        help='Esse comando só pode ser utilizado por membros da equipe de infraestrutura e serve para incluir novos cargos aos que serão adicionados a novos membros ao entrarem no servidor. É fundamental que o nome do cargo que você selecionou esteja exatamente igual ele é no Discord.\n\nVocê pode adicionar um ou mais cargos, separando-os com " | " (caso um dos cargos possua " | " em seu nome, coloque-o como " \| ").\ne.g.: ">addRolesOMJ NOME_CARGO_1 | NOME_CARGO_2 | NOME_CARGO_3"',
    ...
    async def addRolesOMJ(self, ctx, *roles):
        ...
        if 'infra' not in [role.name for role in ctx.author.roles] or not roles:
    ...
    async def removeRolesOMJ(self, ctx, *roles):
        ...
        if 'infra' not in [role.name for role in ctx.author.roles]:
            await reactToMessage(self.bot, ctx.message, ['🙅‍♂️', '❌', '🙅‍♀️'])
